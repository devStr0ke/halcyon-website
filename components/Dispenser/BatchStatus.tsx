...
const DispenserStatus = () => {
    ...
    const { startTimestamp, endTimestamp, price, priceInCoins, supply, left, testCoin } = dispenser;
    ...
    if (batchOrNot === BatchOrNot.SuiSupply) {
        ...
        headline = `Mint a Random Bottle for ${priceInCoins/1000000000} $${coin}, hurry up there is only ${days} day(s), ${hours} hour(s), ${minutes} minute(s), ${seconds} second(s) left!`
    } else if (startTimestamp !== 0) {
        headline = `Next batch opens in ${days} day(s), ${hours} hour(s), ${minutes} minute(s), ${seconds} seconds`
    } else {
        headline = "There is no batch open or planned at the moment, join our community to get more options!"
    }
